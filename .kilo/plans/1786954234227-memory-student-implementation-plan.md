...
### README_submission.md — Nội dung cần cập nhật (bổ sung E08 + E10)

README đã commit thiếu 2 phần mà LAB.md §5.2 + §4.6 yêu cầu:
- E08: recency/conflict handling trong long-term
- E10: compaction trong short-term

**Nội dung README_submission.md cần được sửa (append phần E08+E10 trước dấu guardrail):**
...
## Cau 4: Token reduction so voi full source context, vi sao no-memory co reduction cao nhung hit rate thap?
Memory-enabled: 14.2% reduction, 100% hit rate (11/11). No-memory: ~68% reduction nhưng chỉ 18.2% hit rate (2/11). No-memory "tiết kiệm" token bằng cách trả về rỗng — cheap nhưng incorrect. Token reduction chỉ có giá trị khi kết hợp với hit rate; dropping toàn bộ context giống như không trả lời.

## E08: Recency trong long-term
E08 (lan-lab17, TypeScript cho BLUEBIRD-42) PASS 1693.3ms. Retrieved USER_SUMMARY đã reflect đúng override: "TypeScript with NestJS cho BLUEBIRD-42, Python not permitted" — recency/conflict rule hoạt động. System message tạo ra conflict với preference đã biết (Python), nhưng Context Block của Zep đã ưu tiên fact mới nhất trong graph.

## E10: Compaction trong short-term
E10 PASS (0.3ms). Buffer giữ 6 messages + 2 durable notes + 8 compactions. SESSION_SUMMARY chứa constraint REVIEW-DEADLINE-1600 (FRI 16:00) được giữ nguyên trong DURABLE_NOTES dù 8 lượt filler messages đã được compact. Compaction giữ constraint quan trọng, loại bỏ filler.
...
**Word count:** ~295 từ (under 400 limit).
...
| README_submission.md | ✅ (committed, needs fix) | Repo root — 327 words, missing E08+E10 |

### ⚠️ Artefact Needs Fix
| Item | Issue | Fix |
|------|-------|-----|
| `README_submission.md` | Missing E08 (recency) + E10 (compaction) sections | Append below before guardrail line |

### Verdict: **READY TO SUBMIT after README fix**

The README_submission.md exists and is committed but is **missing required E08 + E10 analysis** (LAB.md §4.6 requires these for the 6-point analysis scoring). The complete corrected content is in the next section — an implementation agent can write it and re-commit.

### Final Steps Before Submission
1. Overwrite `README_submission.md` with complete version (lines 610+ of this plan)
2. Verify word count ≤400: `wc -w README_submission.md`
3. Verify: `git status --short` (no `.env`, no `data/golden_eval.json`)
4. Commit + push:
```bash
cd /Users/apple/Documents/GitHub/AIInAction/Day17-Track3-2A202601044-TranChiVu
git add README_submission.md
git commit -m "Fix README_submission: add E08 recency + E10 compaction sections"
git push
```

---

## Complete README_submission.md Content (Final Version)

> **Copy everything below to `README_submission.md` (overwrite), then commit.**
...
**Word count verification:** ~295 từ (under 400 limit)